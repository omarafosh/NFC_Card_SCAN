...
function getTerminalConfig() {
    ...
    return {
        id: process.env.TERMINAL_ID || null,
...
async function enrollTerminal() {
    log('🆕 New machine detected. Starting auto-enrollment...');

    try {
        // 1. Get first available branch
        const { data: branches, error: bError } = await supabase
            .from('branches')
            .select('id, name')
            .limit(1);

        if (bError || !branches.length) throw new Error('No branches found in database. Manual setup required.');
        const defaultBranch = branches[0];

        // 2. Generate credentials
        const newSecret = crypto.randomUUID();
        const terminalName = `مكنة - ${os.hostname().substring(0, 15)}`;

        log(`🔹 Registering at: ${defaultBranch.name}`);
        ...
        log(`✅ Machine enrolled successfully! ID: ${newTerminal.id}`);
...
async function startReader() {
    ...
    console.log('\n==================================================');
    console.log('🚀 NFC Reader Station - PLUG & PLAY v0.2.2');
    console.log('==================================================');

    // 🔄 Auto-Enrollment Check
    if (!localConfig.id) {
        localConfig = await enrollTerminal();
    }
    ...
    if (error || !terminal) {
        log(`⚠️ Terminal ID ${localConfig.id} invalid or deleted. Clearing config...`);
        if (fs.existsSync(CONFIG_FILE)) fs.unlinkSync(CONFIG_FILE);
        log('Restart the script to auto-enroll as a new machine.');
        process.exit(1);
    }
